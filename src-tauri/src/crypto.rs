...
pub fn verify_signed_message(signed_msg: &SignedMessage) -> Result<bool, String> {
    // Decode public key
    let public_key_bytes = general_purpose::STANDARD.decode(&signed_msg.public_key)
    ...
    // Decode signature
    let signature_bytes = general_purpose::STANDARD.decode(&signed_msg.signature)
...
mod tests {
    ...
    fn test_generate_identity() {
        ...
        assert_eq!(general_purpose::STANDARD.decode(&public_key).unwrap().len(), 32);
    }
